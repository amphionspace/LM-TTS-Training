...
def reference_mel(path, seconds):
    ...
    if not len(waveform) or not torch.isfinite(waveform).all():
        raise ValueError(f"Empty or non-finite reference audio: {path}")
    if sr != 24000:
        waveform = AF.resample(waveform, sr, 24000)
    samples = round(seconds * 24000)
    # Fixed length avoids padding bias in official pooling. Cropping is
    # deterministic, so data-loader RNG cannot perturb checkpoint resume.
    if len(waveform) < samples:
        waveform = waveform.repeat((samples + len(waveform) - 1) // len(waveform))
    start = (len(waveform) - samples) // 2
    return mel_spectrogram(waveform[start:start + samples].unsqueeze(0),
                           n_fft=1024, num_mels=128, sampling_rate=24000,
                           hop_size=256, win_size=1024, fmin=0, fmax=12000)[0].T.contiguous()
