...
每次生成一段语音。当前实现没有多 speaker 条件控制；后续混合多说话人数据时需添加 speaker/reference 条件。
...
精确恢复要求 world size、batch、累积步数、数据、模型、学习率计划等一致；不支持中途改卡数的精确恢复。
...
完整模型和真实音频的检查结果另外记录于 `VALIDATION.md`。
