...
支持单机多卡 FSDP2、断点续训、TensorBoard、验证 loss 和 ASR WER/CER。LJSpeech 用于诊断；当前主线为 Emilia2 英语、中文各约 500 小时独立 short 的非流式预训练。

当前已暂停在可恢复的第 1000 步 checkpoint，代码已统一为 padding-free。四卡两步恢复测试已通过，见 [当前进展与恢复说明](docs/training/restart-progress.md)。
...
NPROC_PER_NODE=4 bash scripts/run_train.sh --config runs/emilia-en-zh-pretrain-1000h/baseline-config.yaml --resume latest
