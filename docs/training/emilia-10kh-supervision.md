...
## 2026-09-10 07:23 UTC 单次巡检：分配器修复后推进至14920，19000停止点已生效

- 依据/权限/进度：已读故障处置表、本文最新主会话诊断与07:12交接、manual、快照20260910T072129Z.json、最新review 20260910T065129Z.md和status（上轮退出码0）。manual.active=false，phase=allocator-recovery-verified-stop19000-applied，交接观察至14630。上一快照14510（旧失败PID）→本轮07:21:31快照14870（当前PID）→07:22:47现场14900→07:23:17现场14920。当前PID实际从14500恢复后已推进420次更新，未停在旧故障点；不同PID的重复日志不混作本次更新。
- 身份与停止点：training-process.json PID104206/start86461963、launcher104205/start86461960/PPID1与/proc匹配；四rank104235–104238/start86462113、PPID104206均R，launcher/torchrun为S。命令为项目torchrun、四卡、原config、--resume latest；torchrun及四rank的/proc环境均为PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True。run/config.yaml与源配置均为max_steps19000/schedule_steps38539。ack.status=applied、pid104206，与当前身份及allocator-recovery/verification.json一致，主会话已验证130次更新；本轮不再次重启、不重写ack。training-exit.json不存在。
- 原故障证据：已实读main-diagnostic/root-cause.json及完整oom-traceback.log。受控复现PID4015636在14510后loss.backward/FlashAttention backward报GPU3 OOM，申请5.85GiB、设备空闲5.66GiB、PyTorch allocated50.42GiB、reserved未分配21.60GiB。反向分配失败已确认，大量未分配保留空间支持碎片机制；DataLoader Aborted发生于清理，是后续异常，不当作首因。主会话据此启用expandable_segments并保留finally前输出原始traceback的修正。root-cause.json的“full4GPU validation pending”是诊断时历史字段，已被较晚verification.json的130步通过及本轮新进展更新，不能误判当前仍未验证。
- 恢复核验：以allocator-recovery/launch.json的log_offset1866549读取当前PID日志段，initialized记录明确为step14500/epoch0/next_batch14500、world_size4。独立比较当前14510、14520与reference-metrics.json，两步的first/residual CE、grad_norm、samples/frames/tokens及两组LR逐值相等；当前段所有训练数值有限，未出现Traceback/OOM/Non-finite/Error/Aborted/terminate called。快照收录的旧OOM与ChildFailedError仍属先前进程，日志全部保留。
- 当前PID14510–14900的40个每10步当步值：first_ce最小/中位/最大/末值1.182595/1.249260/1.311952/1.275424；residual_ce 5.807248/5.889940/5.927884/5.859587；clip前grad_norm 0.371340/0.414331/0.510218/0.415230。主干LR7.417383362e-5→7.283511543e-5、新参数2.225215009e-4→2.185053463e-4，原38539步cosine连续下降。稍后14920 first/residual CE1.206634/5.866442、grad0.395082、LR7.276593887e-5/2.182978166e-4，同样有限。
- 动态batch/吞吐：上述40点帧填充94.9%–99.8125%、中位98.4229%；token填充92.0917%–98.1611%、中位95.1806%；samples314–398。step范围2.008142–2.705435s、中位2.151431s；音频秒/墙钟秒687.062–931.806、中位870.842；data_wait范围0.0001572–0.0004558s、中位0.0002736s；无>3s慢步，含恢复早期点仍与先前稳定阶段接近，未见持续供数下降。rank0 peak allocated最高56.8375GiB。14920为samples399、step2.302229s、吞吐824.384、等待0.0002730s。以上为日志采样的当步值，不是包含启动/评估的窗口均值。
- 资源：本轮GPU0–3占用63969/62101/62241/62081 MiB（各81920，约60.63–62.47GiB），利用率99/98/68/19%，compute-apps仅本run四rank。相比原故障前约73–79GiB占用下降，但不将一次显存快照视为所有后续batch/评估的容量保证。主机已用235GiB、available1.7TiB、无swap；磁盘可用582553.15GiB，未见新容量不足或其他GPU任务竞争。
- Checkpoint/val：latest仍step-00014500，COMPLETE、progress14500/epoch0/next_batch14500、world_size4、scheduler.last_epoch14500及LR7.420789722e-5/2.226236917e-4一致。14000/14500签名相同，各有四distcp约2.093GB、.metadata1424887字节、四rng各14613字节；14500 metadata SHA256与停止前一致，未编辑或清理checkpoint。最近val仍14500 first/residual CE1.298761/5.895098，15码本CE3.716236–6.653376均有限。当前尚未到15000保存/评估点，没有新checkpoint不属于缺失故障。

|最近完整14500，每模式4EN/4ZH、greedy/min_new_frames=2|EN基础WER/CER|EN英语规范化WER/CER|ZH WER/CER|截断/零帧/仅2帧|
|---|---|---|---|---|
|speaker_only|0.444444/0.391608|0.5/0.384615|1.125/0.595238|0/8、0/8、0/8|
|icl|0.194444/0.104895|0.194444/0.104895|1.125/0.440476|0/8、0/8、0/8|

- 生成核验：重读两summary和16条metrics，全部35个14500评估文件SHA256与停止前一致；既有WAV有限、24kHz单声道、时长及固定目标/参考配对核验继续适用，本轮未重复解码或试听。没有15000新产物，不把旧summary当成分配器修复后生成验收。英语规范化SO00把didn't展开，句WER1.25→1.5；SO05为1.5→1.75。ZH仍有繁简和数字口径差异，8句诊断不能代替512条验证loss或据此调参。
- 输出问题未新增也未解决：SO00 2.4s/目标1.81s、SO05 2.72s/目标1.62s、SO06 5.04s/目标3.56s均有额外内容，英语规范化WER分别1.5/1.75/0.333333；ICL00与04分别1.52s/目标1.81s、1.92s/目标2.3s，未见两帧输出但仍有换词。长ZH ICL03 7.12s/目标6.28s、CER0.527778，上次波形核验未见历史多秒零段；ICL12 3.6s/目标5.28s、CER0.482759，尾部仍为“微久微久”等错误。无截断不等于内容完整；后续15000重点复核双模式生成及分配器下评估显存。
- 判断/处理/剩余：保持已验证的分配器设置、预算6000/9000、workers16/prefetch2、原token loss、LR与19000停止目标。理由是当前进程持续更新、原失败位置已跨过、数值与恢复轨迹通过核验，无新需干预故障。本轮不启停训练、不改代码/配置或ack；继续让现进程运行到19000，由训练器执行最终保存/val/双模式生成。最终19000 COMPLETE、val/双summary与include-speaker冻结检查尚未完成，final-verification.json不存在，未执行最终冻结检查或写passed=true。
- 命令/修改/结果：cat/tail读取约定、manual/ack/快照/review/status及主会话根因/验证/完整traceback；Python核对/proc身份及仅分配器环境变量、按启动offset统计当前日志、数值finite与参考精确对照、checkpoint结构/签名/SHA256、源/运行配置、35评估文件SHA256及summary/metrics；nvidia-smi GPU/compute-apps、free -h、shutil.disk_usage；最后复核14920、manual=false、exit/final不存在。全部成功；唯一人工写入为本段追加。未改训练代码/配置/数据/ack，未运行额外测试、启停或发信号、timer/Codex/subagent、提交推送/发消息或额外清理。本次巡检结束。

## 2026-09-10 07:54 UTC 单次巡检：15000保存及双模式评估完成，ICL两条两帧近静音复发

- 依据与进度：已读故障处置表、本文、manual、快照20260910T075129Z.json、最新review 20260910T072129Z.md及status（上轮巡检退出码0）。manual.active=false，分配器修复已交接；ack为applied/PID104206，停止目标19000、schedule_steps38539已生效，无需重复重启。上一快照14870/现场14920→本轮07:51:30快照15340→07:52:21现场15360→07:54:39复核15430，当前正常训练；15000保存、val、两种生成summary均完成，修复后已跨过完整保存评估周期。
- 身份与错误：torchrun104206/start86461963、launcher104205/start86461960/PPID1、四rank104235–104238/start86462113/PPID104206与/proc及原config/--resume latest命令一致，launcher/torchrun为S、rank为R。torchrun及四rank环境仍为PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True。以本PID启动offset1866549扫描当前日志，无新Traceback/OOM/Non-finite/Error/Aborted/terminate called；training-exit.json不存在。历史反向OOM及worker清理错误继续保留，不当作当前故障。
- 近半小时14880–15360共49个每10步当步值均有限：first_ce最小/中位/最大/末值1.184385/1.246925/1.287484/1.241269；residual_ce 5.807248/5.863902/5.909551/5.862746；clip前grad_norm 0.368145/0.409239/0.532963/0.431528。主干LR7.290424203e-5→7.123180693e-5，新参数2.187127261e-4→2.136954208e-4，按原38539步cosine连续下降。稍后15430 first/residual CE1.256240/5.880819、grad0.434028，主干LR7.098565726e-5，同样有限。
- 动态batch与吞吐：上述49点帧填充94.6542%–99.9333%、中位98.3250%；token填充91.8278%–98.4194%、中位95.4139%；global samples326–404。step范围2.034522–2.577456s、中位2.164049s；音频秒/墙钟秒742.344–911.821、中位873.405；data_wait范围0.0002488–0.0004887s、中位0.0002852s。无>3s记录，较上轮2.151431s/870.842/0.0002736s接近，未见持续供数下降。rank0 peak allocated最高56.9679GiB；以上为稀疏当步值，不是含生成评估的窗口平均。
- Checkpoint与验证：latest=step-00015000，COMPLETE时间07:27:49.657244 UTC；progress={step:15000,epoch:0,next_batch:15000}、world_size4、scheduler.last_epoch15000、LR7.248873684e-5/2.174662105e-4。14500/15000签名相同，各有四distcp约2.093GB、.metadata1424887字节、四rng各14613字节齐全；仅结构核查，未另行试加载15000。14000由训练器keep_checkpoints=2轮转，本巡检未清理或编辑metadata。15000 val first_ce1.299879，较14500的1.298761轻微上升约0.001118；residual_ce5.882938较5.895098下降，15码本CE均有限（3.709927–6.638210）。不将单次轻微波动作为调整LR依据。

|15000完整评估，每模式4EN/4ZH、greedy/min_new_frames=2|EN基础WER/CER|EN英语规范化WER/CER|ZH WER/CER|截断/零帧/仅2帧|
|---|---|---|---|---|
|speaker_only|0.777778/0.300699|0.777778/0.300699|1.125/0.630952|0/8、0/8、0/8|
|icl|0.222222/0.188811|0.222222/0.188811|1.125/0.690476|0/8、0/8、2/8|

- 完整评估核验：实读15000两summary、16条metrics和16WAV，全部24kHz单声道、有限、时长与metrics匹配；目标ID/文本/speaker_reference_id/reference_text及policy与14500一致。SO/ICL summary分别07:34:13/07:39:04完成，随后训练继续更新，非评估卡死。两种英语口径本轮相同；SO EN规范化WER由0.5升至0.777778，而CER从0.384615降至0.300699，受sample05长字母串及分词影响，不能只取一项宣布改善/恶化。ICL EN WER0.194444→0.222222，ZH CER0.440476→0.690476，包含两帧近静音及繁简/ASR差异。只有8句诊断，未试听、不扩大解释为全部验证集趋势。
- ICL两帧近静音复发：EN00从14500的19帧/1.52s变为2帧/0.16s、目标1.81s、比0.088398，RMS5.7930e-5、peak0.0003052、零比例34.9479%，ASR空、WER/CER1；ZH04从24帧/1.92s变为2帧/0.16s、目标2.3s、比0.069565，RMS0.0002498、peak0.0023499、零比例44.8958%，ASR“字幕by索兰娅”、CER1。后者在如此短且低能量片段上的ASR不能当作实际生成了完整字幕句子。两者均随后EOS、未截断，符合min2下界但没有有效目标内容；零帧0不能遮蔽实际失败。这两目标在13000/13500已分别出现过相同类型异常，因此不能仅因这次发生在分配器修复后就归因于分配器。
- SO短EN05继续异常：43帧/3.44s、目标1.62s、比2.123457，相比14500的34帧/2.72s加长；RMS0.058080、peak0.282654、最长零段末尾0.006083s，无长静音，ASR为“T-B-T-B-T-S-T-B-T-S-T-E-D-D-A-B-A-L-E.”，WER4.5/CER1.384615，其中15次词插入。此次错误包含字母串重复；CER低于上一轮并不表示恢复。同目标ICL19帧/1.52s、WER0.25/CER0.076923。既有目标/参考核查未发现输入损坏，本轮配对仍一致，无证实可直接修复的新实现错误，不自行加大最短长度或改训练目标。
- 长ZH ICL03连续10500–15000十次无截断/历史多秒零段：85帧/6.8s、目标6.28s、比1.082803，RMS0.036938、零比例14.6734%、最长开头零段0.435333s；ASR“又漆了半個小時,現在是下午2點50,漆了大概10公里,先生預測電量是80%”，CER0.611111，仍含错词、繁简和数字差异。SO03为82帧/6.56s、RMS0.037305、最长末尾零段0.05725s、CER0.666667，开头仍错。未重现历史32s截断或多秒静音，但不能宣称全部生成根因已解决。
- 其他内容：SO00缩至26帧/2.08s/目标1.81s但WER1（“I mean in liquid spares.”）；SO06 54帧/4.32s/目标3.56s、WER0.166667仍额外“Opposite”；SO02 WER0.25，McLaughlin变“big laugh link”等替换。SO01 16帧/1.28s/目标0.98s、CER1.333333，SO04 28帧/2.24s/目标2.3s、CER0.769231。ICL01 11帧/0.88s/CER0.666667，02 WER0.0625、06 WER0.166667；ICL12 42帧/3.36s/目标5.28s/比0.636364，ASR“相信你跟師傅都看過我寄給你們的絕威 威丟威丟”、CER0.655172，尾部仍错/缺。SO12为52帧/4.16s/目标5.28s、CER0.379310。无截断不等于完整。
- 资源与判断：07:52:21附近GPU占用63975/62125/62425/62165 MiB（各81920），利用率78/23/29/17%，compute-apps仅本run四rank；训练继续推进，单次rank利用率差不当作停滞。主机已用243GiB、available1.7TiB、无swap；磁盘可用582485.39GiB。修复后训练、保存、teacher-forcing验证及双模式生成均实际完成且没有新OOM；本轮保持expandable_segments、预算6000/9000、workers16/prefetch2、原token loss、LR及19000停止点。生成异常有实际复发，需继续逐目标跟踪15500的ICL00/04、SO05及ICL12，不能凭小样本噪声改超参，也不能将两帧/重复内容记成已解决。
- 最终验收与操作：当前尚未到19000，training-exit/final-verification均不存在，未执行最终冻结检查或写passed=true；仍需最终19000 COMPLETE、val/双summary及include-speaker冻结检查全部通过。执行cat/tail读取约定/manual/ack/快照/review/status；Python核对/proc身份及allocator环境、分段日志finite/统计/错误、checkpoint结构/签名/时间、配置、磁盘；nvidia-smi/free；soundfile/numpy实读16WAV、时长/能量/零段和metrics配对，最后复核15430/manual=false/ack=applied/PID104206/退出文件及summary时间。核验命令均成功；一次工具脚本在执行前因字符串换行语法失败，未运行任何shell命令或写入，随后修正并成功追加本段，非训练错误。唯一人工写入为本段追加。未改训练代码/配置/数据/ack、启停或发信号、运行额外测试、timer/Codex/subagent、提交推送/发消息或额外清理。本次巡检结束。

## 2026-09-10 08:23 UTC 单次巡检：推进至15910，ICL两帧输出恢复但SO短英文重复加重

- 依据/进度/权限：已读故障处置表、本文、manual、快照20260910T082129Z.json、最新review 20260910T075129Z.md及status（上一巡检退出码0）。manual.active=false；ack=applied/PID104206，运行配置max_steps19000/schedule_steps38539，停止点无需再次加载。上一快照15340/现场15430→本轮08:21:31快照15860→08:22:22现场15890→08:23:08复核15910，训练持续推进。15500保存、val和双summary完整；尚未到16000评估点，不等待下一评估才结束本轮。
- 身份与错误：torchrun104206/start86461963、launcher104205/start86461960/PPID1、四rank104235–104238/start86462113/PPID104206及原config/--resume latest命令均与/proc一致，launcher/torchrun为S、rank为R。torchrun及四rank环境仍为PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True。以当前PID启动offset1866549读取日志，无新Traceback/OOM/Non-finite/Error/Aborted/terminate called，training-exit.json不存在；旧OOM和主动停止日志未清理，也未误算成当前错误。
- 近半小时15350–15890共55个每10步当步值全部有限：first_ce最小/中位/最大/末值1.181872/1.241269/1.285238/1.243116；residual_ce 5.813967/5.865799/5.892089/5.869620；clip前grad_norm 0.362153/0.415166/0.530139/0.394406。主干LR7.126692589e-5→6.935484647e-5、新参数2.138007777e-4→2.080645394e-4，原38539步cosine连续下降。稍后15910 first/residual CE1.256876/5.868987、grad0.434706、主干LR6.928344153e-5，同样有限。
- 动态batch/吞吐：上述55点帧填充94.7375%–99.6958%、中位98.3042%；token填充91.0111%–97.6333%、中位95.2194%；samples315–395。step范围1.983816–2.319725s、中位2.135289s；音频秒/墙钟秒817.683–937.795、中位881.517；data_wait范围0.0002130–0.0004831s、中位0.0002828s。无>3s慢步，较上轮2.164049s/873.405/0.0002852s接近，供数正常；rank0训练peak allocated最高56.8729GiB。以上为稀疏当步值，不是包含保存/评估的窗口平均。
- Checkpoint/val：latest=step-00015500、COMPLETE时间07:57:11.564421 UTC；progress={step:15500,epoch:0,next_batch:15500}、world_size4、scheduler.last_epoch15500、LR7.073895899e-5/2.122168770e-4。15000/15500签名相同，各有四distcp约2.093GB、.metadata1424887字节、四rng各14613字节，结构完整；本轮未试加载15500。14500由训练器keep_checkpoints=2轮转，本巡检未额外清理或编辑metadata。15500 val first/residual CE1.289002/5.872413，较15000的1.299879/5.882938下降，15码本CE均有限（3.694640–6.627551）。

|15500完整评估，每模式4EN/4ZH、greedy/min_new_frames=2|EN基础WER/CER|EN英语规范化WER/CER|ZH WER/CER|截断/零帧/仅2帧|
|---|---|---|---|---|
|speaker_only|0.527778/0.664336|0.555556/0.664336|1.375/0.428571|0/8、0/8、0/8|
|icl|0.083333/0.041958|0.083333/0.041958|1.25/0.535714|0/8、0/8、0/8|

- 完整评估核验：两summary分别08:03:19与08:08:19完成，随后训练继续；实读16条metrics及16WAV，均24kHz单声道、有限、音频时长与metrics一致。目标ID/文本/speaker_reference_id/reference_text及policy与15000相同；ICL02/06实际时长4.0800417/3.5200417s与metrics相符，不强行按帧数舍掉一个采样点。未试听。SO06的That's经英语规范化展开为that is，句WER0.25→0.333333，使SO汇总两种口径有差异，不能混用。
- ICL上轮两帧目标均恢复：sample00从2帧/0.16s恢复17帧/1.36s、目标1.81s、比0.751381，RMS0.061216、peak0.346710、最长开头零段0.047125s，ASR“The liquid spears.”、WER0.25/CER0.166667（目标含重复the，目标自身ASR曾同样省去重复词）。sample04从2帧/0.16s恢复24帧/1.92s、目标2.3s、比0.834783，RMS0.094140、peak0.403076、最长零段0.0195s，ASR“其实人家毕竟钱使了这么多年”、CER0.153846，仍有钱使/坚持替换。仅2帧计数2/8→0/8，说明本轮恢复了有效内容；历史已有反复，不宣称过早结束根因消失。
- SO短EN05连续异常且时长加重：15000为43帧/3.44s，本轮62帧/4.96s、目标1.62s、比3.061728，RMS0.060530、peak0.432373、零比例0.1000%、最长零段仅一个采样点，排除长静音解释。ASR“Chida bodhishek chikush dhvayi dhvayi dhvayi dhvayi dhvayi dhvayi dhvayi dhvayi dhvayi.”，WER3/CER5.384615，长重复仍明显。同目标ICL19帧/1.52s、ASR“W.A. Flinders-Uni.”、WER/CER0。SO EN汇总规范化WER0.777778→0.555556，但CER0.300699→0.664336，不能据WER下降宣布改善；生成重复从分隔字母转为长词串时，词/字符指标的计数变化尤需结合原文和时长。
- 长ZH ICL03连续10500–15500十一次无历史多秒零段或截断：78帧/6.24s、目标6.28s、比0.993631，RMS0.036397、零比例11.9558%、最长开头零段0.447208s；ASR“又騎了半卡雪石先是下午2點50,騎了大約10公里,線上於今天20%”、CER0.75，包含实质错词/数字错误及繁简差异，时长接近目标不能说明内容正确。SO03 88帧/7.04s、RMS0.037341、最长零段0.0065s，CER0.305556较上轮0.666667低，但ASR改用简体和中文数字也影响口径，且“喝水的半小时…先生鱼的电量…”仍错。
- 其他内容：SO00 28帧/2.24s/目标1.81s、WER0.75仍有“I never known”额外内容；SO06 54帧/4.32s/目标3.56s、规范化WER0.333333，额外“That's it”；SO02 WER0.0625。SO01 15帧/1.2s/目标0.98s，ASR“莊 谢谢你 白头后”、WER3/CER0.833333，空格分词影响ZH WER；SO04 CER0.615385。ICL01 11帧/0.88s/CER0.5、02 WER0、06 WER0.166667。ICL12 41帧/3.28s/目标5.28s、比0.621212，ASR“相信你跟師傅都看我 我寄給你們那句 video”、CER0.448276，尾部仍不完整；SO12 53帧/4.24s/CER0.413793。两模式0截断不等于无缺词或重复。
- 资源/判断：08:22:22附近GPU0–3占用63975/62285/62425/62165 MiB（各81920），利用率18/22/23/19%，compute-apps仅本run四rank；快照与现场step持续增加，不能凭单次利用率较低判停滞。主机已用243GiB、available1.7TiB、无swap；磁盘可用582365.09GiB。训练、供数、保存验证正常，当前无需运行时修复；保持expandable_segments、预算6000/9000、workers16/prefetch2、原token loss、LR及19000停止点。SO05重复和ICL12缺尾仍须跟踪，既有输入核查、同目标ICL与固定配对差异不足以证明新数据/实现故障，不盲改LR或解码参数。8句汇总不能代表全部512条验证集质量；16000继续看同目标及EOS/时长。
- 最终验收与命令：当前未到19000，training-exit/final-verification不存在，未执行最终冻结检查或写passed=true；最终仍需19000 COMPLETE、val/双summary与include-speaker冻结检查实际通过。执行cat/tail读取约定/manual/ack/快照/review/status；Python核对/proc身份及allocator环境、当前PID日志finite/统计/错误、checkpoint结构/签名/时间、配置/磁盘；nvidia-smi/free；soundfile/numpy实读16WAV、时长/能量/零段、逐句metrics及配对，最后复核15910、manual=false、ack=applied/PID104206、退出文件和summary时间。全部成功；唯一人工写入为本段追加。未改训练代码/配置/数据/ack、启停或发信号、运行额外测试、timer/Codex/subagent、提交推送/发消息或额外清理。本次巡检结束。


## 2026-09-10 08:31 UTC 排队sqrt实验及监控交接

- 用户明确授权当前run在19000步停止后自动开始同设置的sqrt-loss训练。已写next-run.json及顶部巡检指令，准备新run的launch.py、manual-observation.json、supervision-prompt.md和独立巡检文档；新配置只改loss_reduction=sqrt和输出目录，仍为四卡、19000停止、38539调度。新run从原assembled model、step0开始，不续接token-loss checkpoint。
- 监控脚本新增顺序交接：旧run最终验收通过但下一项启动未验证时继续巡检；新run至少20次更新通过并写含PID/start_ticks的verified ack后，外层切换目录、重置步数统计、沿用同一session继续监督。只有当前run正常退出、最终checkpoint/val/双summary和冻结核验通过，才允许首次启动新run。
- 三项监控交接回归通过，覆盖未确认/身份不匹配不能交接、前序未验收不能交接、同一session切换后继续巡检；启动器隔离测试通过，覆盖验收缺失和前序仍存活时不启动，以及通过时无resume启动四卡sqrt。证据sqrt-handoff/preflight-verification.json与monitor-tests.log。
- 确认旧监控441407/start80639542无活动巡检子进程后，仅替换监控为4145999/start87000429以加载交接逻辑，session仍01a08697-ba78-7cd3-bb9e-b01eba12d724。训练PID104206/start86461963保持运行。旧/新监控身份及现场保存sqrt-handoff/monitor-restart.json。新监控立即恢复该session读取排队指令；此处尚不宣称已收到ack或sqrt已启动。
- 每1800秒巡检；发现前序完成并验收后在同次巡检启动，不额外延后一轮。新run后续记录位于docs/training/emilia-10kh-sqrt-supervision.md。没有提前启动sqrt、没有提交或推送。

- 08:33 UTC 确认：原持久化session已原子写next-run-ack.json，status=received、received_at=2026-09-10T08:33:03.593559+00:00；已核对仅loss/output差异，并明确等待当前19000最终验收与全部rank退出。sqrt仍未启动；当前训练PID104206身份存活匹配。此次排队通知已被实际接收，后续started/verified以真实启动记录为准。


## 2026-09-10 08:39 UTC 单次巡检：sqrt请求已排队，16000完整评估后继续训练

- 授权与交接：已读故障处置表、本文最新记录、manual、最新快照20260910T083138Z.json、上次review 20260910T082129Z.md及status，并读取新增next-run请求。按授权，本轮08:33:03.593559 UTC已原子写next-run-ack.json，status=received、run_dir与请求绝对路径一致；08:38:48复核仍为received。收到仅表示排队，sqrt training-process.json不存在，未提前启动。重新比较两源配置，仅train.loss_reduction从默认token改为sqrt、train.output改为独立sqrt目录；max_steps19000、schedule_steps38539及其余设置保持。当前run正常退出且19000 checkpoint、val、双summary、include-speaker冻结核验全部通过后，须在同次巡检用已准备launch.py从assembled/step0启动sqrt，真实20次更新及PID/start_ticks核验通过才写verified，外层负责沿用session交接。本轮未到19000，不执行后继启动或最终验收。
- 当前进度与身份：上次快照15860/现场15910→本次08:31快照16000→08:38:06现场16010→08:38:48复核16030。manual.active=false；停止ack仍applied/PID104206。torchrun104206/start86461963、launcher104205/start86461960/PPID1、四rank104235–104238/start86462113/PPID104206及本项目原config/--resume latest命令均匹配/proc；torchrun和四rank均使用expandable_segments:True。运行配置max_steps19000/schedule_steps38539，已有当前PID恢复证据，不重复重启。当前PID日志offset1866549后无新Traceback/OOM/Non-finite/Error/Aborted/terminate called；training-exit/final-verification不存在。保留旧故障日志，不将旧OOM算作当前异常。
- 训练指标：此次提前交接通知距上次巡检约10分钟，统计15870–16010共15个新增稀疏训练日志点，非完整半小时均值。first_ce最小/中位/最大/末值1.193049/1.224262/1.285514/1.244361，residual_ce 5.815597/5.856532/5.886132/5.856532，clip前grad_norm 0.376368/0.403020/0.470975/0.392524，全部有限。主干LR6.942621120e-5→6.892582059e-5、新参数2.082786336e-4→2.067774618e-4沿原38539调度连续下降。最后16030 first/residual CE1.179011/5.836009、grad0.423355、LR6.885417856e-5/2.065625357e-4，同样有限。
- 吞吐与资源：上述15点step 2.037205–2.254769s、中位2.166143s；音频秒/墙钟秒834.785–937.795、中位869.100；data_wait 0.0002547–0.0003874s、中位0.0002812s。帧填充95.2750%–99.6042%、中位98.8417%；token92.6056%–97.3556%、中位96.3333%，samples315–405，rank0 peak最高56.7674GiB。没有>3s慢步，供数/吞吐与上轮相近。GPU占用63975/62285/62425/62165 MiB，各81920，利用率23/22/22/20%，compute-apps仅四训练rank；主机已用242GiB、available1.7TiB、无swap，磁盘可用582215.46GiB。评估已完整后步数继续，不因评估期间step16000暂留或单次低GPU利用率判断卡死。
- Checkpoint与val：latest=step-00016000，COMPLETE时间08:26:20.738747 UTC，progress={step:16000,epoch:0,next_batch:16000}、world_size4、scheduler.last_epoch16000、LR6.896162699e-5/2.068848810e-4。保留15500/16000且signature相等；各四distcp约2.093GB、distributed/.metadata1424887字节、四rng各14613字节及metadata齐全，结构核验通过，未试加载。15000按训练器keep_checkpoints=2轮转，本巡检未额外清理。16000 val first_ce1.289532，较15500的1.289002微升0.000530；residual_ce5.862609，较5.872413下降，15码本CE均有限（3.683185–6.619501）。不能概括为两项都下降。

|16000完整生成，每模式4EN/4ZH、greedy/min_new_frames=2|EN基础WER/CER|EN英语规范化WER/CER|ZH WER/CER|截断/零帧/仅2帧|
|---|---|---|---|---|
|speaker_only|0.416667/0.356643|0.416667/0.356643|1.125/0.464286|0/8、0/8、0/8|
|icl|0.111111/0.125874|0.111111/0.125874|1.0/0.452381|0/8、0/8、1/8|

- 评估完整性：SO/ICL summary分别08:32:23/08:37:24完成，随后更新至16030。实读两summary、16条metrics和16个generated.wav，均24kHz单声道、有限、时长匹配；目标ID/文本/speaker_reference_id/reference_text及policy与15500一致。此次英语两种规范化汇总一致；ZH仍受繁简、数字和分词口径影响，8句不能代表全验证集质量。未试听音频。
- ICL EN00两帧近静音再次复发：15500为17帧/1.36s，本轮2帧/0.16s，目标1.81s、比0.088398，RMS4.91588e-5、peak0.00024414、零比例49.349%、最长开头零段0.023917s，ASR空、WER/CER1。满足min2、随后EOS且未截断，仍未生成有效内容，不能由零帧0宣称正常。ZH04本轮23帧/1.84s、目标2.3s、RMS0.080645，没有两帧复发，但ASR“岂世然必见先识了这么多年”、CER0.615385，较15500的0.153846更差。既有13000/13500/15000反复记录说明问题未解决，不能盲增最短长度或归因于allocator。
- SO EN05较上轮缩短但仍错误：15500的62帧/4.96s→16000的32帧/2.56s，目标1.62s、比1.580247，RMS0.066050、最长末尾零段0.037292s；ASR“Chee by the Tentis, we diswin this.”，WER1.75/CER1.692308，未出现上轮长dhvayi串但远非正确。对应ICL20帧/1.6s、ASR“W.A. Flinders-Uni.”、WER/CER0。SO00则33帧/2.64s/目标1.81s，ASR“Eyedness are now and shins light spears.”、WER1.5/CER1.277778，额外/替换内容仍明显；SO06 4.24s/目标3.56s、WER0.166667，开头多“A patek”。
- 长ZH ICL03连续10500–16000十二次无历史多秒零段/截断：85帧/6.8s、目标6.28s、比1.082803，RMS0.036388、零比例11.9988%、最长开头零段0.441708s；ASR“又起了半个小时,现在是下午两点五时,起了大概10公里,现在生育的电量是80%”、CER0.333333，仍有起/骑、生育/剩余等错误及数字口径。SO03为84帧/6.72s、RMS0.035693、最长零段0.006917s、CER0.277778，开头内容仍错。ICL12为42帧/3.36s/目标5.28s、比0.636364、CER0.517241，ASR“相信你跟師父都看過我寄給你們的絕威尿videos”，尾部重复video未完整；SO12为56帧/4.48s、CER0.551724，同样错误。无截断不等于内容完整。
- 其他逐句：SO01 14帧/1.12s/目标0.98s、CER0.666667；SO02 4.32s、WER/CER0；SO04 2.32s/目标2.3s、CER0.692308。ICL01 0.96s/目标0.98s、CER0.5、末尾最长零段0.17525s；ICL02 4.16s、05 1.6s、06 3.6s，三条EN均WER/CER0，因此ICL EN错误全部来自00的空输出，汇总不可遮蔽此失败。
- 判断/操作/未解决：训练、保存、teacher-forcing验证及双模式生成正常完成且继续更新，当前无新运行故障，保持原token loss、预算6000/9000、workers16/prefetch2、LR、allocator及19000停止点；不凭8句波动改超参。继续跟踪ICL00过早EOS、SO短句额外内容及ICL12缺尾，现有配对与输入检查没有新证据支持改数据/实现。本轮命令为cat/tail、Python读取/proc身份/环境、当前PID日志统计/有限性/错误、配置差异、checkpoint签名/结构/mtime、磁盘、summary/metrics及soundfile/numpy波形检查，nvidia-smi/free和最后状态复核；检查成功。写入仅08:33原子received ack及本文追加，未改训练代码/配置/数据、启停进程、运行额外测试、创建timer/Codex/subagent、提交推送/发消息或清理。最终19000验收及sqrt启动/20步验证尚未到执行条件，未写passed=true/verified。本次巡检结束。


## 2026-09-10 09:04 UTC 单次巡检：16500 checkpoint/val/SO完成，ICL持续生成

- 依据/权限/进度：已读故障处置表、本文、manual、最新快照20260910T090138Z.json、上次review 20260910T083138Z.md及status（上一轮退出码0）。上次快照16000/现场16030→本次09:01:39快照16500；09:02:48与09:03:41现场仍16500，处于正常生成评估。manual.active=false，stop-at-19000 ack=applied/PID104206，运行配置max_steps19000/schedule_steps38539且已有该PID严格恢复验证，停止点已实际生效，无需重复重启。
- 进程与错误：torchrun104206/start86461963、launcher104205/start86461960/PPID1、四rank104235–104238/start86462113/PPID104206均与/proc身份和原config/--resume latest命令匹配；采样launcher/torchrun为S、rank为R，torchrun及四rank环境均expandable_segments:True。当前PID日志offset1866549后无新Traceback/OOM/Non-finite/Error/Aborted/terminate called；training-exit/final-verification不存在。旧故障日志保留，未将旧OOM算成此次错误。
- 近半小时训练：16010–16500共50个稀疏当步训练记录均有限。first_ce最小/中位/最大/末值1.173378/1.228382/1.284431/1.213630；residual_ce 5.817226/5.849863/5.891963/5.847545；clip前grad_norm 0.369578/0.410265/0.482123/0.427045。主干LR6.892582059e-5→6.715985241e-5、新参数2.067774618e-4→2.014795572e-4沿原38539步调度连续下降，没有因生成波动调LR。
- 吞吐/动态batch：step范围1.987254–2.334007s、中位2.145951s；音频秒/墙钟秒816.690–961.165、中位873.853；data_wait 0.0002348–0.0006967s、中位0.0002800s。帧填充95.3333%–99.7583%、中位97.9021%；token91.6528%–97.7472%、中位95.0333%；samples308–425。rank0 peak allocated最高56.6690GiB，无>3s慢步，和上轮2.166143s/869.100/0.0002812s接近，未见供数退化。这些是每10步采样的当步统计，不含评估耗时的窗口平均。
- 资源：09:02附近GPU占用63975/62285/62425/62165 MiB（每卡81920），利用率0/100/100/100%，compute-apps仅本run四rank；正在生成/后处理且产物继续增加，不由瞬间卡间差异推断卡死。主机已用243GiB、available1.7TiB、无swap；磁盘可用582119.35GiB，无容量压力证据。
- Checkpoint与val：latest=step-00016500，COMPLETE时间08:55:24.242091 UTC，progress={step:16500,epoch:0,next_batch:16500}、world_size4、scheduler.last_epoch16500、LR6.715985241e-5/2.014795572e-4。16000/16500 signature相同；每份各四distcp约2.093GB、distributed/.metadata1424887字节、四rng各14613字节及metadata齐全，结构核验通过，未试加载。15500由训练器keep_checkpoints=2轮转，本巡检未清理。16500 val first/residual CE1.281711/5.851342，较16000的1.289532/5.862609均下降；15码本CE有限，范围3.675417–6.605361。

|评估阶段，每模式4EN/4ZH，greedy/min_new_frames=2|EN基础WER/CER|EN英语规范化WER/CER|ZH WER/CER|截断/零帧/仅2帧|
|---|---|---|---|---|
|16500 speaker_only，8条完整|0.333333/0.251748|0.361111/0.251748|1.625/0.5|0/8、0/8、0/8|
|16000 icl，最近完整8条，历史结果|0.111111/0.125874|0.111111/0.125874|1.0/0.452381|0/8、0/8、1/8|
|16500 icl，本次尚在生成|未汇总|未汇总|未汇总|已检查3条均无截断/零帧/仅2帧；其余待完成|

- 评估产物与口径：本轮重新读16000双summary、16500 SO summary、SO全部8条metrics/WAV与ICL已完成00/01/02的3条metrics/WAV，共11WAV；全部24kHz单声道、有限、时长与metrics匹配，目标ID/文本/speaker_reference_id/reference_text/policy与16000对应样本一致。16500 SO summary于09:01:22.980670完成；ICL00/01/02分别09:01:55/09:02:23/09:03:06落盘，09:03:41尚无ICL summary，说明仍有进展，不将未完成指标记零或宣称双summary完成。未试听。SO00的didn't经英语规范化展开使句WER1.5→1.75、汇总0.333333→0.361111；ZH含繁简、数字和英文混合分词，需结合逐句内容。
- SO短ZH01新增长输出：16000为14帧/1.12s，本轮32帧/2.56s，目标0.98s、比2.612245，RMS0.073181、peak0.558258、零比例15.0488%、最长末尾零段0.154083s；ASR“莊勤現在呆死後 就給小上市人就盡色”、WER2/CER2.666667，存在额外内容，非长静音造成时长。对应本次ICL01 13帧/1.04s/目标0.98s，ASR“煮至奶白色後”、CER0.166667（含後/后差异），RMS0.069045、最长末尾零段0.237083s，未复现SO加长。后续继续跟踪，不依据单句新增波动改训练配置。
- SO短EN05继续缩短且指标较好但仍错：16000的32帧/2.56s→本轮29帧/2.32s，目标1.62s、比1.432099，RMS0.066395、最长零段0.000083s，ASR“G.W. Hay-Flanders-Punit”、WER1/CER0.461538；较上轮WER1.75/CER1.692308下降，但不能宣布完全恢复。SO00仍33帧/2.64s/目标1.81s、RMS0.070027，ASR“I didn't know the liquid speed was showing.”、规范化WER1.75/CER1.111111，额外内容未解决。SO06 52帧/4.16s/目标3.56s，ASR“Ascended the Jeffreeze tube. They open a door and enter a swamp.”、WER0.166667/CER0.227273，开头替换仍在。
- SO长ZH03为73帧/5.84s/目标6.28s，比0.929936、RMS0.040492、最长零段0.015375s、无截断，ASR“夫妻的半个小时现在是下午两点五十 起了大概十公里 现在剩下的电量是百分之”、CER0.194444，开头错误且末尾八十缺失；CER下降不等于句子完整。SO04 30帧/2.4s、CER0.615385；SO12 56帧/4.48s/目标5.28s，ASR“王教贤你跟师傅都看过我寄给你们那句 with a video videos”、WER5/CER0.379310，混合语言/空格造成词插入计数，不能把CER下降解读成无错。SO02 53帧/4.24s、WER/CER0。
- 本轮ICL00已从16000两帧近静音恢复：20帧/1.6s、目标1.81s、比0.883978，RMS0.053292、peak0.331177、最长开头零段0.013458s，ASR“the liquid spears.”、WER0.25/CER0.166667，省去目标重复the，与既有目标自身ASR口径相符。有效内容恢复，但历史反复，不能宣布过早EOS根因解决。ICL02为48帧/3.84s/目标4.8s、WER/CER0。ICL04/05/06/12及长ZH03本轮结果尚未核验，不能沿用16000结果当作16500；最近完整16000 ICL03无历史多秒零段、ICL12缺尾等历史判断保持待新证据更新。
- sqrt排队/最终验收：再次读取next-run.json与next-run-ack.json，run_dir完全匹配，原08:33:03.593559 received确认仍有效，未重复改写时间或升级verified。两源配置差异仍仅loss_reduction从默认token到sqrt、独立output；新run training-process.json不存在。当前未达19000且原训练正常，禁止提前并行启动，保持排队。仍需原run最终19000 COMPLETE、val/双summary、正常退出与include-speaker冻结核验后写final-verification；通过后同次巡检立即从assembled step0启动sqrt并验证20次真实更新再交接，未额外等一轮的授权保持有效。本轮未执行最终冻结检查或写passed=true。
- 判断与命令/修改：保持原token loss、预算6000/9000、workers16/prefetch2、LR、allocator和19000停止点，原因是训练/供数/保存/验证健康，评估正在推进，8句输出差异尚无证据支持新实现/数据修复。执行cat/tail读取约定/记录；Python读取快照/review/status/manual/ack、/proc身份/allocator环境、当前PID日志/有限性/统计、checkpoint结构/签名/mtime、配置差异与磁盘；nvidia-smi/free；soundfile/numpy核对11WAV及metrics；最后复核09:03:41的manual=false/PID104206/无退出和ICL进展。均成功，唯一写入为本段追加，未改代码/配置/数据/ack、启停或发信号、跑额外测试、创建timer/Codex/subagent、提交推送/外发消息或额外清理。本次巡检结束，16500 ICL完整summary及剩余逐句输出由后续巡检继续核验。


## 2026-09-10 09:34 UTC 单次巡检：17000验证完成，SO额外内容继续，ICL生成中

- 依据/状态：已读故障处置表、本文、manual、快照20260910T093138Z.json、上次review 20260910T090138Z.md及status（上一轮退出码0）。上次16500→本轮09:31:39快照17000，09:32:26现场仍17000，正在生成评估。manual.active=false，stop-at-19000 ack=applied/PID104206；运行配置max_steps19000/schedule_steps38539，已有同PID严格恢复验证，停止点已生效，不重复重启。
- 进程/错误：torchrun104206/start86461963、launcher104205/start86461960/PPID1、四rank104235–104238/start86462113/PPID104206均与/proc匹配，命令为原config/--resume latest；launcher/torchrun为S、四rank为R。torchrun及四rank均使用expandable_segments:True。当前PID日志offset1866549后无新Traceback/OOM/Non-finite/Error/Aborted/terminate called。09:33:20复核manual=false/PID104206，training-exit/final-verification均不存在，未将历史OOM/主动停止当作新错误。
- 近半小时训练：16510–17000共50个每10步的当步值均有限。first_ce最小/中位/最大/末值1.140052/1.227541/1.273781/1.198681；residual_ce 5.779494/5.842888/5.884331/5.845113；clip前grad_norm 0.369087/0.404238/0.515173/0.515173。末步梯度为本窗口最大，但仍低于clip阈值1且有限，不能仅因单点上升调整。主干LR6.712358920e-5→6.533678961e-5、新参数2.013707676e-4→1.960103688e-4，按原38539调度连续下降。
- 吞吐/资源：step范围1.955984–2.454508s、中位2.126172s；音频秒/墙钟秒781.289–952.581、中位885.750；data_wait 0.0002379–0.0005373s、中位0.0002860s，无>3s慢步，和上轮2.145951s/873.853/0.0002800s接近。帧填充95.8417%–99.8792%、中位97.9917%；token91.7167%–97.7389%、中位95.2903%；samples296–403，rank0 peak allocated最高56.4841GiB。09:32 GPU占用63975/62285/62425/62305 MiB（各81920），利用率0/100/100/100%，compute-apps仅本run四rank；主机已用244GiB、available1.7TiB、无swap；磁盘可用582036.75GiB。评估产物持续增加，不能由步骤暂留或单卡瞬时空闲判停滞。训练统计不是包含评估时间的窗口均值。
- Checkpoint/val：latest=step-00017000，COMPLETE时间09:24:28.673758 UTC，progress={step:17000,epoch:0,next_batch:17000}、world_size4、scheduler.last_epoch17000、LR6.533678961e-5/1.960103688e-4。16500/17000 signature相同，每份四distcp约2.093GB、distributed/.metadata1424887字节、四rng各14613字节及metadata齐全；结构核验通过，未试加载。16000由训练器keep_checkpoints=2轮转，本巡检未清理。17000 val first/residual CE1.274521/5.845262，较16500的1.281711/5.851342均下降；15码本CE有限，范围3.676496–6.600795，其中码本1略升，不能声称每码本都改善。

|生成评估，每模式4EN/4ZH，greedy/min_new_frames=2|EN基础WER/CER|EN英语规范化WER/CER|ZH WER/CER|截断/零帧/仅2帧|
|---|---|---|---|---|
|16500 speaker_only，已完整、重读summary|0.333333/0.251748|0.361111/0.251748|1.625/0.5|0/8、0/8、0/8（上轮已核）|
|16500 icl，本轮补核完整8条|0.027778/0.020979|0.027778/0.020979|1.25/0.476190|0/8、0/8、0/8|
|17000 speaker_only，完整8条|0.361111/0.370629|0.361111/0.370629|1.375/0.869048|0/8、0/8、0/8|
|17000 icl，生成中|待summary|待summary|待summary|已核3条均无截断/零帧/仅2帧，其余待完成|

- 完整性/进展：16500 ICL summary于09:06:27.176426完成，之后训练推进500步，本轮补核8条逐句与WAV；17000 SO summary于09:31:10.336748完成，8条全部核验。17000 ICL00/01于09:31:41/09:32:08落盘，随后02也已完成；09:33:20为3条，尚无summary，正常处于后续样本生成。合计实读19WAV，均24kHz单声道、有限、时长与metrics匹配；19条目标ID/文本/speaker_reference_id/reference_text/policy与各自前一checkpoint对应样本相同。未试听，不把未完成ICL结果记成零。17000 SO英语基础/规范化汇总相同；其规范化WER与16500同为0.361111，但CER0.251748→0.370629，ZH CER0.5→0.869048，逐句异常不能被WER持平遮蔽，8句仍不能代表整个验证集。
- 16500 ICL补齐：00保持20帧/1.6s、RMS0.053292、WER0.25/CER0.166667；02/05/06 WER/CER均0，分别3.84/1.68/3.92s，因此EN唯一错误为00省略重复the。04为23帧/1.84s/目标2.3s、RMS0.082777、CER0.461538，ASR“其實人家畢竟監視了這麼多年”，监视/坚持仍错且有繁简差异。01为1.04s/CER0.166667、最长末尾零段0.237083s。长ZH03为79帧/6.32s/目标6.28s、比1.006369、RMS0.039640、最长开头零段0.447917s、零比例13.4428%，无截断/历史多秒零段；10500–16500连续十三次未复发该长静音形式。ASR“又騎了半個小時,現在是下午2點50,騎了大概10公里,現在剩下的電量是80%”、CER0.527778，繁简/数字影响明显，不等于音频退化。12为43帧/3.44s/目标5.28s、比0.651515，ASR“相信你跟师傅都看过我 记得你们那卷 微调微调”、CER0.482759，尾部仍错/缺。
- 17000 SO短ZH01连续额外内容：16500为32帧/2.56s，本轮34帧/2.72s，目标0.98s、比2.775510、RMS0.077664、peak0.595032、最长末尾零段0.186625s；ASR“方浅崇右四里滚角四 除手间哪些角色又浮游”、WER2/CER3。非多秒静音拖长，内容明显错误。对应17000 ICL01为11帧/0.88s/目标0.98s、RMS0.084860、最长末尾零段0.117083s，ASR“组织奶杯伺候”、CER0.833333，虽未加长但内容仍错。
- 17000 SO12新增长输出：16500为56帧/4.48s，本轮108帧/8.64s，目标5.28s、比1.636364，RMS0.057355、peak0.486450、零比例0.2103%、最长零段0.002708s；ASR“我只相信你跟师父都看过我寄给你们那卷 VSFVD 只有4S来CDG出一则则CDG要CDG去求出”、WER3/CER0.931034。明显额外串，非静音解释；以EOS结束、truncated=false仍不能称正常。本次ICL12尚未生成完成，不用上一步ICL代替本步对照。继续跟踪该新变化。
- 17000 SO05再次变长：16500的29帧/2.32s→46帧/3.68s，目标1.62s、比2.271605，RMS0.065225、最长末尾零段0.01775s；ASR“Chidebulio, the coast to Cattity, Hidebula Shinnai.”、WER1.75/CER2.692308，前两次缩短未构成持续恢复。SO00本轮30帧/2.4s、目标1.81s，ASR“I just romped the liquid spears.”、WER0.75/CER0.5，仍有额外内容；SO06 47帧/3.76s/目标3.56s、WER0.166667/CER0.159091，开头“Duck in”及Jeffreeze替换。SO02 56帧/4.48s、WER0.0625/CER0.029412，yeah变yak；SO04 28帧/2.24s、CER0.538462。
- 17000 SO长ZH03为85帧/6.8s/目标6.28s、RMS0.041117、最长末尾零段0.034917s，ASR“回去了半廈時,現在是下午兩點五時 騎了大概10公里,現在剩預計電量是80%”、CER0.583333，仍有内容错误与繁简/数字因素；未复发长静音。17000 ICL00已完成18帧/1.44s/目标1.81s、RMS0.057580、最长开头零段0.02525s、ASR“and the liquid spears.”、WER0.25/CER0.166667，继16500后连续两次有效内容，历史两帧问题不宣称根除。ICL02为44帧/3.52s/目标4.8s、WER/CER0。其余ICL03/04/05/06/12及完整summary待后续检查。
- 停止/排队：09:33:20重读next-run请求与ack，绝对run_dir一致、status=received/received_at08:33:03.593559保持，未重写或假报verified。两源配置仍仅默认token→sqrt loss及独立output不同；sqrt training-process不存在，未提前并行启动。当前17000未到19000，final-verification不存在，未执行最终冻结检查。仍须19000 COMPLETE、val/双summary、正常退出、include-speaker冻结核验实际通过，再在同次巡检立即启动sqrt fresh assembled/step0，至少20次真实更新验证后交接；授权和停止目标保持。
- 判断/执行/未解决：训练、供数、保存、val健康，ICL产物持续推进，保持现配置（token loss、预算6000/9000、workers16/prefetch2、原LR、allocator及19000停止点）。SO01连续加长、SO12新增长串、SO05反复及ICL12缺尾均如实保留；当前配对正确、WAV可读且无新实现故障证据，不凭8句波动盲改超参/解码长度或重启。执行cat/tail、Python读取快照/manual/ack/review/status、/proc身份/环境、当前PID日志finite/统计/错误、checkpoint结构/签名/时间、配置差异/磁盘、summary/逐句metrics、soundfile/numpy读取19WAV，nvidia-smi/free及最后状态复核；全部成功。唯一人工写入为本文追加，未修改训练代码/配置/数据/ack、启停发信号、测试、timer/Codex/subagent、提交推送/外发消息或额外清理。本次巡检结束，17000 ICL剩余结果留后续核验。


## 2026-09-10 10:05 UTC 单次巡检：17500验证健康，ICL两帧输出再次出现

- 依据/权限/进度：已读故障处置表、本文、manual、快照20260910T100138Z.json、最新review 20260910T093138Z.md及status（上一轮退出码0）。上次17000→本次10:01:39快照17500，10:03:21现场17500，正在生成评估；manual.active=false，stop-at-19000 ack=applied/PID104206。运行配置max_steps19000/schedule_steps38539与当前PID既有恢复验证一致，停止点无需重启加载。
- 进程/错误：torchrun104206/start86461963、launcher104205/start86461960/PPID1、四rank104235–104238/start86462113/PPID104206与/proc匹配，原config/--resume latest命令正确；launcher/torchrun为S、rank为R，torchrun及四rank均expandable_segments:True。当前PID日志offset1866549后无新Traceback/OOM/Non-finite/Error/Aborted/terminate called。10:04:13复核manual=false/PID104206、training-exit与final-verification不存在。旧故障日志保留，未混入当前错误判断。
- 近半小时训练：17010–17500共50个稀疏当步值全部有限。first_ce最小/中位/最大/末值1.164631/1.219270/1.278772/1.194316；residual_ce 5.756128/5.827502/5.868451/5.822139；clip前grad_norm 0.362902/0.409570/0.460208/0.439120。主干LR6.530013308e-5→6.349563023e-5、新参数1.959003992e-4→1.904868907e-4按原38539步曲线连续下降。
- 吞吐与资源：step 1.947170–2.269421s、中位2.136682s；音频秒/墙钟秒818.041–940.689、中位874.107；data_wait 0.0002493–0.0005043s、中位0.0002807s，无>3s慢步，与上轮2.126172s/885.750/0.0002860s接近。帧填充94.7917%–99.6958%、中位97.9%；token91.4194%–97.3611%、中位95.1153%；samples307–386，rank0 peak allocated最高56.7139GiB。GPU占用63975/62285/62425/62305 MiB（各81920）、利用率0/100/100/100%，compute-apps仅四训练rank；主机已用243GiB、available1.7TiB、无swap；磁盘可用581879.67GiB。训练统计为每10步采样的当步值，不包含保存/评估墙钟耗时。ICL产物持续增加，当前不存在卡死/供数退化证据。
- Checkpoint/val：latest=step-00017500，COMPLETE于09:54:22.382502 UTC，progress={step:17500,epoch:0,next_batch:17500}、world_size4、scheduler.last_epoch17500、LR6.349563023e-5/1.904868907e-4。17000/17500 signature相同，均有四distcp约2.093GB、distributed/.metadata1424887字节、四rng各14613字节及metadata，结构完整，未试加载。16500由训练器keep_checkpoints=2轮转，本巡检未清理。17500 val first/residual CE1.268324/5.836255，较17000的1.274521/5.845262下降；15码本CE全部有限，范围3.665508–6.588806。

|生成评估，每模式4EN/4ZH，greedy/min_new_frames=2|EN基础WER/CER|EN英语规范化WER/CER|ZH WER/CER|截断/零帧/仅2帧|
|---|---|---|---|---|
|17000 speaker_only，已完整、重读summary|0.361111/0.370629|0.361111/0.370629|1.375/0.869048|0/8、0/8、0/8（上轮已核）|
|17000 icl，本轮补核完整8条|0.027778/0.020979|0.027778/0.020979|1.125/0.535714|0/8、0/8、1/8|
|17500 speaker_only，完整8条|0.25/0.195804|0.25/0.195804|1.125/0.559524|0/8、0/8、0/8|
|17500 icl，尚在生成|待summary|待summary|待summary|已核5条：截断0、零帧0、仅2帧2；其余待核|

- 完整性/进展：17000 ICL summary于09:36:17.414993完成，随后训练继续500步；本轮补核该步8条metrics/WAV。17500 SO summary于10:00:26.513081完成，8条metrics/WAV全核；17500 ICL已核00–04五条。共21WAV全部24kHz单声道、有限、时长与metrics匹配，目标ID/文本/speaker_reference_id/reference_text/policy均与各自上一checkpoint相同。未试听。10:04:13目录已增至6条，新增05于10:03:54.876701落盘，本轮只核对其存在/时间，未审其内容或音频；尚无summary，不将部分ICL汇总当作完整结果。两步已读summary英语基础/规范化口径相同；ZH繁简/数字/空格会影响WER/CER，8句不能代表全验证集质量。
- 17000 ICL补齐揭示ZH04两帧复发：2帧/0.16s、目标2.3s、比0.069565、RMS3.72072e-5、peak0.00021362、零比例70.9115%、最长开头零段0.081583s，ASR“字幕by索兰娅”、CER1。如此短且近静音的ASR不能当作实际说出了该完整字幕句。16500同目标1.84s，而17000丢失有效内容。00为18帧/1.44s、WER0.25/CER0.166667；02/05/06为3.52/1.6/3.6s且WER/CER0，EN唯一错误来自00的and/the替换。01为0.88s、CER0.833333；12为41帧/3.28s/目标5.28s、比0.621212，ASR“相信你更是不都看過我寄給你們那一卷 為了videos”、CER0.517241，内容仍错/缺，未出现SO12的8.64s增长串。
- 17500 ICL00和04再次只有两帧：00为0.16s/目标1.81s，比0.088398、RMS7.63781e-5、peak0.00024414、零比例25.1302%、最长开头零段0.004208s，ASR空、WER/CER1；04为0.16s/目标2.3s，比0.069565、RMS0.00160358、peak0.0126953、零比例58.3333%、最长末尾零段0.057625s，ASR仍“字幕by索兰娅”、CER1。04能量比17000高，但仅两帧且低于正常同目标量级，仍未形成有效目标句，不能由非零波形或未截断声称恢复。两者均min2后EOS；00继16500/17000有效输出后再退化，04连续17000/17500失败，问题仍反复，不盲增最短长度。
- 长ZH ICL03：17000为81帧/6.48s/目标6.28s、RMS0.039751、最长开头附近零段0.43925s、CER0.333333；17500同为81帧/6.48s，比1.031847、RMS0.035526、peak0.241394、零比例14.6920%、最长开头零段0.511708s，CER0.527778。17500 ASR“又起了半個小時,現在是下午2點50,起了大概10公里,現在剩餘的電量是80%”，与17000内容接近但繁简也影响CER。10500–17500连续十五次没有历史多秒零段/截断，不能把约0.5s开头零段等同历史15s静音，也不宣称全部生成问题根除。17500 ICL01为1.04s/目标0.98s、ASR“就在奶白色后”、CER0.333333；02为3.76s/目标4.8s，ASR省去you know、WER0.125/CER0.102941。05/06/12和完整summary本轮尚未内容核验。
- 17500 SO过长输出缩短，但仍有错误：ZH01从17000的34帧/2.72s缩到15帧/1.2s，目标0.98s、比1.224490、RMS0.078927、最长零段0.165375s，ASR“掌聲鼓勵一下”、WER/CER1，时长回落并未恢复正确内容。SO12从108帧/8.64s缩到55帧/4.4s，目标5.28s、比0.833333、RMS0.053066、最长末尾零段0.017s，ASR“我相信你跟师父都看过我寄给你们 那卷飞了videos”、CER0.344828，长串消失但尾部仍不完整。SO05从46帧/3.68s缩到26帧/2.08s，目标1.62s、比1.283951、RMS0.068517、最长零段一个采样点，ASR“to W.A. Flendis Uni.”、WER0.5/CER0.384615，仍有额外to/替换。历史已有多次反复，不能仅据本轮缩短宣布解决。
- 其他17500 SO：00为25帧/2s/目标1.81s、ASR“on the liquid spars.”、WER0.5/CER0.222222；02为55帧/4.4s/目标4.8s、WER0.1875/CER0.161765，McLaughlin变McLean且缺词；06为48帧/3.84s/目标3.56s、WER0.166667/CER0.181818，开头Touch in及Jeffreeze仍错。03为86帧/6.88s/目标6.28s、RMS0.036584、最长末尾零段0.016917s、CER0.638889，ASR“飛起了半個小時,現在是2點50km了,大概10公里 先剩餘的電量是80%”，仍有内容/单位错误与繁简因素；04为26帧/2.08s/目标2.3s、CER0.615385，开头错且末尾年缺失。未截断不代表语句完整。
- 排队/最终验收：10:04:13重读next-run及ack，绝对run_dir完全匹配，status仍received、received_at08:33:03.593559保持；sqrt training-process.json不存在。重新比较两源配置仍仅默认token→sqrt loss及独立output不同。当前未到19000，未运行最终冻结核验或写passed=true/verified，未提前并行启动sqrt。原run最终19000 COMPLETE、val/双summary、正常退出与include-speaker冻结核验通过后，仍按授权在同次巡检从assembled step0启动sqrt、核验20次真实更新后交接，无需再次许可。
- 判断/命令/修改：保持原token loss、预算6000/9000、workers16/prefetch2、LR、allocator与19000停止点。训练、供数、保存和验证正常，ICL产物持续更新；生成过早EOS及错误内容仍未解决，但配对和波形检查未给出可直接修复的新实现/数据故障证据，不凭8句调整超参。执行cat/tail读取约定/记录；Python核对快照/manual/ack/review/status、/proc身份/环境、当前PID日志finite/统计/错误、checkpoint结构/签名/时间、运行配置/源配置差异/磁盘、summary/逐句metrics，soundfile/numpy读21WAV；nvidia-smi/free及10:04:13最终状态复核。全部成功；唯一写入为本段追加，未改训练代码/配置/数据/ack、启停发信号、运行测试、timer/Codex/subagent、提交推送/外发消息或额外清理。本次巡检结束，17500 ICL剩余内容及完整summary由后续巡检核验。


## 2026-09-10 10:34 UTC 单次巡检：18000 checkpoint/val完成，ICL两帧问题持续

- 依据/进度/权限：已读故障处置表、本文、manual、快照20260910T103138Z.json、最新review 20260910T100138Z.md及status（上一轮退出码0）。上次17500→10:31:39快照18000；10:32:30现场仍18000，处于正常生成评估。manual.active=false，stop-at-19000 ack=applied/PID104206；运行配置max_steps19000/schedule_steps38539与当前PID已有严格恢复验证一致，不重复重启。
- 身份与错误：torchrun104206/start86461963、launcher104205/start86461960/PPID1、四rank104235–104238/start86462113/PPID104206均与/proc匹配，原config/--resume latest命令正确；launcher/torchrun为S、rank为R，torchrun和四rank均expandable_segments:True。当前PID日志offset1866549后无新Traceback/OOM/Non-finite/Error/Aborted/terminate called。10:33:24复核manual=false/PID104206，training-exit/final-verification均不存在；历史故障日志保留且未误当新错误。
- 近半小时训练：17510–18000共50个稀疏当步值均有限。first_ce最小/中位/最大/末值1.170807/1.212265/1.288263/1.183003；residual_ce 5.776817/5.830825/5.860465/5.803177；clip前grad_norm 0.359159/0.412082/0.494699/0.415087。主干LR6.345864455e-5→6.163959757e-5、新参数1.903759336e-4→1.849187927e-4，沿原38539步调度连续下降。
- 吞吐/资源：step 1.991224–2.293512s、中位2.166682s；音频秒/墙钟秒826.012–939.152、中位868.293；data_wait 0.0002380–0.0005266s、中位0.0002834s，无>3s慢步，与上轮2.136682s/874.107/0.0002807s接近。帧填充95.1333%–99.8958%、中位98.1104%；token91.5306%–97.8222%、中位95.4222%；samples314–385，rank0 peak allocated最高56.6428GiB。GPU占用63975/62285/62425/62305 MiB（各81920），利用率60/57/67/26%，compute-apps仅四训练rank；主机已用244GiB、available1.7TiB、无swap；磁盘可用581693.76GiB，无容量压力。以上为每10步当步统计，不是含保存/评估的窗口均值；ICL产物继续增加，步骤暂留不构成卡死。
- Checkpoint/val：latest=step-00018000，COMPLETE时间10:23:21.236315 UTC，progress={step:18000,epoch:0,next_batch:18000}、world_size4、scheduler.last_epoch18000、LR6.163959757e-5/1.849187927e-4。17500/18000 signature相同；每份四distcp约2.093GB、distributed/.metadata1424887字节、四rng各14613字节及metadata齐全，结构核验通过，未试加载。17000由训练器keep_checkpoints=2轮转，本巡检未清理。18000 val first/residual CE1.267848/5.825030，较17500的1.268324/5.836255下降，first仅下降约0.000476；15码本CE有限，范围3.660651–6.575919。

|生成评估，每模式4EN/4ZH，greedy/min_new_frames=2|EN基础WER/CER|EN英语规范化WER/CER|ZH WER/CER|截断/零帧/仅2帧|
|---|---|---|---|---|
|17500 speaker_only，已完整、重读summary|0.25/0.195804|0.25/0.195804|1.125/0.559524|0/8、0/8、0/8（上轮已核）|
|17500 icl，本轮补核完整8条|0.194444/0.181818|0.194444/0.181818|1.0/0.630952|0/8、0/8、2/8|
|18000 speaker_only，完整8条|0.472222/0.286713|0.472222/0.286713|1.375/0.5|0/8、0/8、0/8|
|18000 icl，尚在生成|待summary|待summary|待summary|已核6条：截断0、零帧0、仅2帧2；其余待核|

- 完整性/进展：17500 ICL summary于10:05:17.007165完成，随后训练继续500步，本轮补核8条metrics/WAV。18000 SO summary于10:29:23.770702完成，8条metrics/WAV全核；18000 ICL00–05六条已核。共22WAV均24kHz单声道、有限、时长匹配；目标ID/文本/speaker_reference_id/reference_text/policy与各自上一步评估相同。10:33:24 ICL仍6条无summary，其中03/04/05分别10:32:00/10:32:26/10:32:57落盘，有明确进展，不把部分结果当完整汇总。未试听。18000 SO英语基础/规范化WER/CER虽相同，但规范化替换/插入计数为7/9、基础为8/8，口径仍区分记录；ZH繁简、数字和空格影响指标，8句不能代表全验证集。
- 17500 ICL补齐：00/04两帧异常和03长句结果与上轮已读一致。05为19帧/1.52s/目标1.62s、RMS0.071347、最长末尾零段0.056125s，ASR“W.I. Flinders-Uni”、WER0.25/CER0.076923；06为46帧/3.68s/目标3.56s、RMS0.150436、WER/CER0。12为42帧/3.36s/目标5.28s、比0.636364、RMS0.055958、最长末尾零段0.047667s，ASR“相信你跟師傅都看過我寄給你們的覺得”、CER0.655172，尾部video缺失，较17000仍未恢复完整内容。汇总EN错误来自00空输出、02缺you know、05的I/A替换；两帧计数必须单独记录，不能被0截断遮蔽。
- 18000 ICL两帧持续：EN00为2帧/0.16s/目标1.81s、比0.088398、RMS4.60142e-5、peak0.00024414、零比例54.6615%、最长开头零段0.027583s，ASR空、WER/CER1，继17500连续两次无有效内容。ZH04为2帧/0.16s/目标2.3s、比0.069565、RMS0.00145586、peak0.0246277、零比例57.8646%、最长末尾零段0.056958s，ASR“字幕by索兰娅”、CER1，继17000/17500连续三次两帧失败；如此短的低能量片段上的ASR不可当作实际完整字幕句。两者均min2后EOS，不代表正确生成。历史反复并未解决，未盲增最短长度或归因于allocator。
- 18000 ICL其他已完成样本：01为12帧/0.96s/目标0.98s、ASR“逐漸奶白色後”、CER0.5、RMS0.067327、最长末尾零段0.160292s；02为52帧/4.16s/目标4.8s、WER/CER0；05为19帧/1.52s/目标1.62s、ASR“W.A. Flinders-Uni”、WER/CER0、RMS0.056917。长ZH03为87帧/6.96s/目标6.28s、比1.108280、RMS0.038437、零比例11.5344%、最长开头零段0.445875s，无截断/历史多秒零段，10500–18000连续十六次未复发该长静音形式。ASR“又起了半個小時,現在是下午2點50,起了大概10公里,現在剩餘點量是80%”、CER0.555556，仍有起/骑、缺的及繁简/数字因素；不宣称全部生成根因解决。06/12与完整summary尚待核验。
- 18000 SO短EN05再加长：17500的26帧/2.08s→40帧/3.2s，目标1.62s、比1.975309、RMS0.072120、最长末尾零段0.038458s；ASR“2W. Hey, Flenders. Be good up. You and this.”、WER2.25/CER1.538462，额外内容未解决，同步ICL05为1.52s且WER/CER0。SO00为26帧/2.08s/目标1.81s，ASR“I know in the liquid spears.”、WER0.75/CER0.388889；SO02为55帧/4.4s/目标4.8s、WER0.1875/CER0.132353，尾部变成Hey, yeah；SO06为50帧/4s/目标3.56s、WER0.166667/CER0.113636，开头多Up及Jeffreeze替换。SO EN汇总0.25→0.472222并不对应训练loss故障，不据此改LR。
- 18000 SO中文：01为13帧/1.04s/目标0.98s，ASR“早期耐擺社後”、CER1、RMS0.073769、最长零段0.000875s，17500以来未复发此前2.56/2.72s增长串，但内容仍错。12为55帧/4.4s/目标5.28s、RMS0.052644、最长末尾零段0.036875s，ASR“我们先请你跟师父都看过 我寄给你们那绝位 视频 视频”、CER0.655172，未复发17000的8.64s串，仍有替换与尾部内容错误。03为84帧/6.72s/目标6.28s、RMS0.038399、最长零段0.012042s、CER0.25，ASR“就这半个小时先是下午两点五时 起了大概十公里 先剩余的电量是百分之八十”，简体/中文数字也使CER较17500低，开头仍错。04为30帧/2.4s/目标2.3s、CER0.615385，ASR“愛惜人家必須先時了這麼多年”。无截断不代表语句完整。
- 停止/排队：10:33:24再次核对next-run与received ack，绝对run_dir匹配、received_at08:33:03.593559保持，未重写或升级verified；sqrt training-process.json不存在。两源配置仍仅默认token→sqrt loss及独立output不同，当前18000未到19000，不提前并行启动。最终仍需19000 COMPLETE、val/双summary、正常退出、include-speaker冻结核验全部实际通过才写final-verification；通过后同次巡检立即用准备好的launch.py从assembled step0启动sqrt并验20次真实更新后交接。本轮未执行最终冻结核验或写passed=true。
- 判断/命令/修改：保持token loss、预算6000/9000、workers16/prefetch2、原LR、allocator及19000停止点，原因是训练/供数/保存/验证正常且ICL产物继续生成，当前没有新运行故障或可直接修复的实现/数据证据。两帧过早EOS、SO额外内容、ICL12缺尾仍未解决并持续记录，不能凭8句盲改超参。执行cat/tail读取约定/记录；Python核对快照/manual/ack/review/status、/proc身份/环境、当前PID日志finite/统计/错误、checkpoint结构/签名/时间、运行配置/源差异与磁盘、summary/逐句metrics，soundfile/numpy读22WAV；nvidia-smi/free及最后状态复核。全部成功，唯一写入为本文追加。未改训练代码/配置/数据/ack、启停发信号、跑测试、timer/Codex/subagent、提交推送/外发消息或额外清理。本次巡检结束，18000 ICL剩余结果由后续巡检核验。


## 2026-09-10 11:04 UTC 单次巡检：18500双模式评估完整，保持19000停止与sqrt排队

- 依据/状态：已读故障处置表、本文、manual、快照20260910T110138Z.json、最新review 20260910T103138Z.md与status（上一轮退出码0）。上次18000→11:01:39快照18500，11:02:38现场18500；11:03:12 ICL summary完成，11:03:32最后训练日志仍18500，处于评估刚收尾阶段，不判卡死。manual.active=false、stop-at-19000 ack=applied/PID104206；run/config.yaml为max_steps19000/schedule_steps38539，与当前PID已有严格恢复验证一致，不重复重启。
- 身份/错误：torchrun104206/start86461963、launcher104205/start86461960/PPID1、四rank104235–104238/start86462113/PPID104206与/proc匹配，命令为原config/--resume latest；launcher/torchrun为S、rank为R。torchrun及四rank均expandable_segments:True。当前PID日志offset1866549后无新Traceback/OOM/Non-finite/Error/Aborted/terminate called；11:03:32复核manual=false/PID104206、training-exit/final-verification均不存在。历史故障日志未算作新错误。
- 近半小时18010–18500共50个每10步当步值全部有限：first_ce最小/中位/最大/末值1.136760/1.207929/1.258878/1.198706；residual_ce 5.767464/5.807452/5.856226/5.847932；clip前grad_norm 0.357355/0.406796/0.490594/0.389668。主干LR6.160234749e-5→5.977194099e-5、新参数1.848070425e-4→1.793158230e-4按原38539曲线连续下降。
- 吞吐/资源：step 1.952176–2.293020s、中位2.137104s；音频秒/墙钟秒830.717–943.570、中位880.601；data_wait 0.0002403–0.0005905s、中位0.0002784s，无>3s慢步，接近上轮2.166682s/868.293/0.0002834s。帧填充94.2542%–99.7208%、中位98.325%；token91.2528%–97.7972%、中位95.1431%；samples307–395，rank0 peak allocated最高56.7262GiB。GPU占用63975/62285/62425/62305 MiB（各81920），利用率59/62/68/26%，compute-apps仅四训练rank；主机已用244GiB、available1.7TiB、无swap，磁盘可用581550.44GiB。无供数/容量故障证据，稀疏当步统计不包含保存/评估墙钟耗时。
- Checkpoint/val：latest=step-00018500，COMPLETE时间10:52:16.939862 UTC，progress={step:18500,epoch:0,next_batch:18500}、world_size4、scheduler.last_epoch18500、LR5.977194099e-5/1.793158230e-4。18000/18500 signature相同，每份四distcp约2.093GB、distributed/.metadata1424887字节、四rng各14613字节及metadata完整；本轮仅结构核验，未试加载。17500由训练器keep_checkpoints=2轮转，本巡检未清理。18500 val first/residual CE1.259500/5.817720，较18000的1.267848/5.825030下降，15码本CE有限（3.655748–6.569315）。

|完整生成，每模式4EN/4ZH，greedy/min_new_frames=2|EN基础WER/CER|EN英语规范化WER/CER|ZH WER/CER|截断/零帧/仅2帧|
|---|---|---|---|---|
|18000 speaker_only，重读summary|0.472222/0.286713|0.472222/0.286713|1.375/0.5|0/8、0/8、0/8（上轮已核）|
|18000 icl，本轮补核完整8条|0.194444/0.174825|0.194444/0.174825|1.125/0.559524|0/8、0/8、2/8|
|18500 speaker_only|0.333333/0.328671|0.333333/0.328671|0.875/0.583333|0/8、0/8、0/8|
|18500 icl|0.138889/0.069930|0.138889/0.069930|1.25/0.654762|0/8、0/8、1/8|

- 完整性：18000 ICL summary于10:34:19.465937完成，本轮补核全部8条metrics/WAV；18500 SO/ICL summary分别10:58:21.994669/11:03:12.903686完成，全部16条metrics/WAV核验。共24WAV均24kHz单声道、有限、时长匹配，目标ID/文本/speaker_reference_id/reference_text/policy与各自前一步评估相同。18500 ICL02实际4.0800417s与metrics一致，不擅自舍去一个采样点。未试听。两步英语汇总两口径相同；SO EN WER下降但CER上升，ZH WER下降但CER上升，不能挑单指标声称整体改善；繁简/数字/分词影响仍在，8句不代表全验证集。
- 18000 ICL补齐：此前00/04两帧与长03结果不变；06为47帧/3.76s/目标3.56s、ASR“In the Jeffreeze Tube, they open the door and enter a swap.”、WER0.25/CER0.159091、RMS0.139569、最长末尾零段0.04925s。12为44帧/3.52s/目标5.28s、比0.666667、ASR“相信你跟师父都看过我寄给你们的剧 videos”、CER0.379310、RMS0.054674，尾部仍不完整，不能由CER下降称恢复。
- 18500 ICL00恢复有效输出：19帧/1.52s/目标1.81s、比0.839779、RMS0.063057、peak0.347473、最长开头零段0.046083s，ASR“the liquid spears.”、WER0.25/CER0.166667，省去重复the。此前17500/18000两帧后本次恢复，但历史反复不宣称根因已解决。ZH04则连续17000/17500/18000/18500四次两帧失败：本次2帧/0.16s/目标2.3s、比0.069565、RMS0.000854273、peak0.0191345、零比例61.6667%、最长末尾零段0.057167s；ASR“字幕by索兰娅”、CER1。低能量0.16s上的ASR不等于实际生成完整字幕句，min2后EOS且未截断仍无有效目标内容，不盲增下界。
- 18500 ICL其他内容：01为11帧/0.88s/目标0.98s、ASR“组织奶白色后”、CER0.333333、RMS0.076310、最长末尾零段0.094833s。02为51帧/4.0800417s/目标4.8s、WER/CER0；05为18帧/1.44s/目标1.62s、ASR“W.A. Flynd is unique.”、WER0.75/CER0.461538，较18000错误增多；06为45帧/3.6s、ASR“In the Jefferies tube, they open a door and enter a swamp.”、WER0.083333/CER0.022727。12为42帧/3.36s/目标5.28s、比0.636364、RMS0.056247、最长末尾零段0.03625s，ASR“相信你跟師傅都看過 借給你們的建威爾 威迪爾威迪爾”、CER0.724138，替换/缺词和尾部错误持续。
- 长ZH ICL03为80帧/6.4s/目标6.28s、比1.019108、RMS0.037543、零比例15.0254%、最长开头零段0.445375s，连续10500–18500十七次无历史多秒零段/截断；ASR“右骑的半小时先是下午2点50,骑的大概是公里,显色域的电量是80%”、CER0.527778，仍错且十公里内容缺失，时长接近目标不能说明内容完整。SO03为89帧/7.12s/目标6.28s、RMS0.039958、最长零段0.038958s，ASR“过去半个小时,现在是下午两点五十 其他大概十公里 现在剩余的电量是百分之八十”、WER0.5/CER0.138889，后两段匹配较好但开头仍错；中文数字/简体口径亦影响CER。
- 18500 SO短ZH01再次增长串：18000的13帧/1.04s→35帧/2.8s，目标0.98s、比2.857143、RMS0.067514、peak0.461456、零比例11.4628%、最长末尾零段0.091792s；ASR“放棄熟知乃白色后 使用球技能球球为它能分纹”、WER2/CER2.833333，非静音拖长。此前16500/17000同类型增长串在17500/18000缩短后又复发。SO05仍40帧/3.2s/目标1.62s、比1.975309、RMS0.056397、最长零段0.000167s，ASR“Chudabha takis buddhaf, buddh takis winnaund.”、WER1.5/CER2.461538，WER较18000低但CER更高，不能说恢复；同目标ICL虽短也出现内容错误。
- 其他18500 SO：00为27帧/2.16s/目标1.81s、ASR“I known in the liquid spears.”、WER0.75/CER0.444444；02为59帧/4.72s/目标4.8s、WER0.0625/CER0.044118，末尾you dou-替换；06为48帧/3.84s/目标3.56s、WER0.166667/CER0.090909，额外Ups和swap替换。04为27帧/2.16s、CER0.769231，开头“阿姨實驗家…”错误。12为53帧/4.24s/目标5.28s、ASR“王上仙你跟师傅都看过我寄给你们那就要威士威迪欧威迪欧”、CER0.586207，未复发17000的8.64s串，但内容仍错。无截断不等于完整或正确。
- 停止/后继：11:03:32重读next-run与ack，绝对run_dir匹配、status=received/received_at08:33:03.593559保持；sqrt training-process.json不存在。两源配置仍仅默认token→sqrt loss与独立output不同。当前18500尚余500次更新，未完成19000最终验收，不提前启动sqrt、未运行冻结检查或写passed=true/verified。最终19000 checkpoint、val/双summary、正常退出与include-speaker冻结核验通过后，按授权在验收同次巡检立即启动sqrt fresh assembled/step0并核验至少20次真实更新后交接；本轮不等待剩余训练完成。
- 判断/操作：保持原token loss、预算6000/9000、workers16/prefetch2、LR、allocator与19000停止点，原因是训练/供数/保存/验证健康，双模式产物实际完成，无新运行故障或可直接修复的实现/数据证据。ZH04持续两帧、SO额外内容及ICL12缺尾均明确未解决，不用8句波动盲改参数。执行cat/tail、Python读取快照/manual/ack/review/status、/proc身份/环境、当前PID日志finite/统计/错误、checkpoint结构/签名/时间、运行配置/源差异/磁盘、summary/逐句metrics，soundfile/numpy读24WAV，nvidia-smi/free及11:03:32最终状态复核；全部成功。唯一写入为本文追加，未改训练代码/配置/数据/ack、启停发信号、测试、timer/Codex/subagent、提交推送/外发消息或额外清理。本次巡检结束。


## 2026-09-10 11:35 UTC 最终19000验收通过，顺序启动sqrt

- 依据/状态：已读故障处置表、本文、manual、快照20260910T113138Z.json、最新review 20260910T110138Z.md及status（上一轮退出码0）。上次18500→11:31:38快照19000。manual.active=false，停止ack=applied/PID104206；运行max_steps19000/schedule_steps38539已实际执行，没有19000之后的训练日志。当前PID自offset1866549后无新Traceback/OOM/Non-finite/Error/Aborted/terminate called。最终val/双模式生成完成后，training-exit.json记录PID104206于11:33:05.091954正常退出，exit_code=0；旧launcher104205/start86461960、torchrun104206/start86461963和四rank104235–104238/start86462113全部/proc消失，未发信号。
- 最后500次更新：18510–19000的50个稀疏当步值全部有限。first_ce最小/中位/最大/末值1.157157/1.204762/1.286328/1.183068；residual_ce 5.751588/5.798060/5.858054/5.836265；clip前grad_norm 0.349529/0.403061/0.473489/0.441478。主干LR5.973449172e-5→5.789593019e-5、新参数1.792034752e-4→1.736877906e-4保持原38539曲线，未修改训练目标或提前调整LR。
- 吞吐/资源：step 1.987095–2.449714s、中位2.140298s；音频秒/墙钟秒763.942–938.863、中位880.714；data_wait 0.0001101–0.0005351s、中位0.0002791s，无>3s慢步，与上一窗口接近。帧填充93.9708%–99.6708%、中位98.775%；token90.7333%–97.9583%、中位95.9097%；samples316–398，rank0 peak最高56.4398GiB。11:32:42评估清理时GPU各约61–62GB、0%利用率，随后全部退回1MiB/0%，compute-apps空；主机清理时已用208GiB、available1.7TiB、无swap，磁盘可用581365.41GiB。统计为每10步当步值，不含评估墙钟时间。
- 最终checkpoint：latest=step-00019000，COMPLETE时间11:21:13.103006 UTC，progress={step:19000,epoch:0,next_batch:19000}、world_size4、scheduler.last_epoch19000、LR5.789593019e-5/1.736877906e-4。18500/19000 signature相同，每份四distcp约2.093GB、distributed/.metadata1424887字节、四rng各14613字节及metadata齐全；18000由训练器keep_checkpoints=2轮转，本巡检未清理。完整训练状态未再做恢复试跑，冻结子集由实际DCP加载核验。
- 最终val：first/residual CE1.256300/5.810860，较18500的1.259500/5.817720下降；15码本CE有限（3.642202–6.564515）。实读验证清单为512条，validate按val_batches遍历后all_reduce，无扩大WER评估。

|19000最终生成，每模式4EN/4ZH，greedy/min_new_frames=2|EN基础WER/CER|EN英语规范化WER/CER|ZH WER/CER|截断/零帧/仅2帧|
|---|---|---|---|---|
|speaker_only|0.611111/0.356643|0.638889/0.370629|1.125/0.666667|0/8、0/8、0/8|
|icl|0.138889/0.132867|0.138889/0.132867|1.625/0.488095|0/8、0/8、1/8|

- SO/ICL summary分别11:27:39.894157/11:32:32.461249完成；实读两summary、16metrics及16WAV，全部24kHz单声道、有限、时长匹配，目标ID/文本/speaker_reference_id/reference_text/policy与18500相同。未试听。SO05的She'd展开使英语句WER4.5→4.75、CER2.692308→2.846154，影响汇总两口径；ZH繁简/数字和混合英文空格仍影响WER/CER。8句诊断结果不能代表全验证集质量，更不能把完成验收等同语音质量已解决。
- 最终ICL00再次两帧：2帧/0.16s/目标1.81s、比0.088398、RMS0.000495253、peak0.00418091、零比例67.3958%、最长开头零段0.0255s，ASR空、WER/CER1，18500短暂恢复未持续。ZH04四次两帧后本次恢复22帧/1.76s/目标2.3s、比0.765217、RMS0.086241、peak0.430206、最长末尾零段0.015875s，但ASR“其實兩家畢竟先捨這麼多年”、CER0.615385仍错。零帧0和EOS达到不等于内容正常。
- 长ZH ICL03为83帧/6.64s/目标6.28s、比1.057325、RMS0.036017、零比例9.4522%、最长开头零段0.074167s，10500–19000十八次未复发历史多秒零段/截断；ASR“又起了半個小時,現在是下午2點50,起了大概10公里,現在剩餘的電量是80%”、CER0.527778，仍有起/骑及繁简/数字口径。ICL12为41帧/3.28s/目标5.28s、比0.621212、RMS0.057928、最长末尾零段0.025167s，ASR“相信你跟師傅都看過我寄給你們 那卷 We are video video”、WER6/CER0.344828，英文分词抬高WER、尾部仍错；不能只据CER降低宣布完整。
- 其他ICL：01为10帧/0.8s/目标0.98s、CER0.666667、ASR“九居奶白伺候”；02为47帧/3.76s/目标4.8s、WER0.0625/CER0.014706，缺a；05为20帧/1.6s/目标1.62s、06为42帧/3.36s/目标3.56s，两条WER/CER0。
- SO短句异常未解决：ZH01为34帧/2.72s/目标0.98s、比2.775510、RMS0.069136、最长零段0.037625s，ASR“咱们先休息一下来台词后 休息为我丢休休想的地方”、CER3.5，连续18500/19000额外串，非长静音拖时长。EN05从40帧/3.2s增至50帧/4s，目标1.62s、比2.469136、RMS0.061151、最长零段0.003542s，ASR“She'd have it to t-t-t-t-t-t-t-t ed real炸 you know when sir”、规范化WER4.75/CER2.846154，明显重复/额外内容。SO00为29帧/2.32s、WER0.75/CER0.444444，额外“I do known”；06为54帧/4.32s、WER0.083333/CER0.181818，额外Opposite。SO02为54帧/4.32s、WER/CER0；SO04为27帧/2.16s、CER0.538462；SO03为86帧/6.88s、RMS0.037954、最长末尾零段0.056125s、CER0.277778，开头和电量句仍错；SO12为57帧/4.56s/目标5.28s、CER0.620690，内容仍有替换/缺词。
- 冻结与最终验收：旧进程全部退出后执行原命令PYTHONPATH=. .venv/bin/python scripts/check_frozen_frontend.py --assembled pretrained/assembled-qwen3-tts-frozen-conditioning --checkpoint runs/emilia-en-zh-dynamic-10000h/checkpoints/step-00019000 --include-speaker，stdout/stderr保存在final-frozen-check.log，进程exit_code=0。结果status=passed、exact_frozen_tensors=81、include_speaker=true、parameters=326313792，text embedding/projection和speaker encoder与assembled逐值一致。日志仅有单进程DCP加载提示，不是失败。11:34:47.522359原子写final-verification.json：passed=true、pid104206/start86461963、step19000，含正常退出、checkpoint/val/双summary、16WAV核验、冻结结果及37个相关产物SHA256，并保留质量限制。原run按用户停止目标完成，生成质量问题没有因验收而被标为解决。
- sqrt即时交接启动：重新读取next-run请求、两配置及新launch.py，确认差异仅loss_reduction=sqrt和output，四卡/max19000/schedule38539/数据/预算/模型结构不变。新run无training-process/自身checkpoint；旧进程全部退出且GPU compute-apps为空。11:35:10.265266使用.venv Python经subprocess.Popen([sys.executable, sqrt绝对launch.py],cwd项目根,start_new_session=True,stdin=DEVNULL,stdout/stderr追加sqrt launcher.log)首次启动，launcher3343586/start88101592，启动器写torchrun3343588/start88101595/started_at11:35:10.292488，命令为sqrt配置且无--resume。新startup-verification.json暂为launching、verified_updates0，ack仍received，不能先报verified。
- 主会话新增观察安排：sqrt manual于11:18设active=true、through_step2000；其predecessor_handoff与sqrt文档明确保留本巡检完成前序、首次启动及20次更新验证的授权。故本次启动继续执行；验证交接后后台在manual=true期间仅只读记录，异常交主会话，不清除此标志。之后详细启动指标追加sqrt独立文档，本文仅补交接摘要。
- 命令/修改/剩余：cat/tail、Python读取约定/manual/快照/review/status与身份/日志/配置/checkpoint/评估，nvidia-smi/free，soundfile/numpy读16WAV，冻结脚本、SHA256、原子final-verification与新启动记录及detached Popen。一次rg附带不存在的qwen3_train/config.py返回2，其余匹配正常，后续直接读取现存train.py/model.py核实，未涉及训练错误或代码写入。未改训练代码/超参/数据/metadata、发信号、提交推送/外发消息、额外清理、创建timer/Codex/subagent。当前剩余为sqrt首次step0初始化与20次真实更新验收，继续在本轮处理。

- 2026-09-10T11:44:49.585438+00:00 顺序交接完成：sqrt PID3343588/start88101595从step0初始化，已验证80次真实更新，普通CE/sqrt CE/梯度有限、LR按原warmup、四rank allocator一致，无新错误。新run startup-verification.json=verified，详细日志/资源/指标追加docs/training/emilia-10kh-sqrt-supervision.md。前序next-run-ack原子升级verified并匹配新PID/start_ticks；外层将沿用同session切换监控目录。sqrt manual=true/前2000步由主会话观察保持不变。原run19000最终验收通过且生成质量限制仍保留，本轮不再改动原run或监控。
